namespace Tsentris {
  ...
  function testGrid(): void {
    ...
    grid.push(cube.mtxLocal.translation, new GridElement(cube));

    let pulled: GridElement = grid.pull(cube.mtxLocal.translation);
    logResult(cube == pulled.cube, "Grid push and pull", cube, pulled.cube, pulled);

    let popped: GridElement = grid.pop(cube.mtxLocal.translation);
    logResult(cube == popped.cube, "Grid pop", cube, popped.cube, popped);

    let empty: GridElement = grid.pull(cube.mtxLocal.translation);
